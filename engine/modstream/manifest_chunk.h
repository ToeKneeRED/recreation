...
// chunk packet is a 12-byte header (u32 total size, u32 total chunk count, u32
// chunk index) followed by this chunk's slice of the manifest bytes. The client
// reassembles them into a buffer; this codec owns the chunk framing so the parse
// and its validation can be unit-tested and fuzzed apart from the net layer.
...
std::vector<u8> EncodeManifestChunk(u32 total_size, u32 total_chunks, u32 chunk_index,
