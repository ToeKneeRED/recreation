...
void RegisterGameAndForms(papyrus::NativeRegistry& reg, SkyrimBindings* bindings) {
  ...
  reg.Register("Form", "GetRaceSpellCount", [bindings](VirtualMachine&, ObjectRef self, Args&) {
    return Value::Int(Resolve(bindings).GetRaceSpellCount(self));
  });
...
void RegisterActor(papyrus::NativeRegistry& reg, SkyrimBindings* bindings) {
  ...
  reg.Register("Faction", "GetFactionFlags", [bindings](VirtualMachine&, ObjectRef self, Args&) {
    return Value::Int(Resolve(bindings).GetFactionFlags(self));
  });
...
void RegisterSkyrimNatives(papyrus::NativeRegistry& reg, SkyrimBindings* bindings) {
  ...
  RegisterWorldExtra(reg, bindings);
